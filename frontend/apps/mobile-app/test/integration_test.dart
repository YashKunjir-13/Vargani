...
void main() {
  group('Full End-to-End Integration & Wiring Suite', () {
    ...
    test(
        'END-TO-END FLOW: Payment creation -> auto Receipt -> WhatsApp dispatch',
        () async {
      ...
      await paymentsNotifier.confirmMatch(newPayment.id,
          matchedBy: 'treasurer-main');

      final paymentsList = container.read(paymentsProvider).value ?? [];
      final updatedPayment =
          paymentsList.firstWhere((p) => p.id == newPayment.id);
      ...
      final generatedReceipt =
          receipts.firstWhere((r) => r.paymentId == newPayment.id);
      ...
      await container
          ...
          .resendWhatsapp(generatedReceipt.id);
      final updatedReceipts = container.read(receiptsProvider).value ?? [];
      final resentReceipt =
          updatedReceipts.firstWhere((r) => r.id == generatedReceipt.id);
      expect(resentReceipt, isNotNull);
    });

    test(
        'LIFECYCLE 2: Non-monetary contribution recorded -> CRCPT- receipt auto-generated',
        () {
      final contributionsNotifier =
          container.read(contributionsProvider.notifier);
      ...
      final crcpt = contributionReceipts
          .firstWhere((r) => r.contributionId == contribution.id);
    ...
    test(
        'LIFECYCLE 3: Bill drafted -> submitted -> self-approval blocked -> approved (different user) -> marked paid',
        () {
      ...
      final submittedBill =
          container.read(billsProvider).firstWhere((b) => b.id == bill.id);
      ...
      // 5. Mark Paid
      billsNotifier.markPaid(bill.id,
          paymentMode: BillPaymentMode.bankTransfer);
      final paidBill =
          container.read(billsProvider).firstWhere((b) => b.id == bill.id);
    ...
    test(
        'COUNTER INDEPENDENCE: RCPT- and CRCPT- counters operate collision-free',
        () async {
      final paymentsNotifier = container.read(paymentsProvider.notifier);
      final contributionsNotifier =
          container.read(contributionsProvider.notifier);
      ...
      final rcpt = (container.read(receiptsProvider).value ?? [])
          .firstWhere((r) => r.paymentId == p.id);
      final crcpt = container
          .read(contributionReceiptsProvider)
          .firstWhere((r) => r.contributionId == c.id);

      expect(rcpt.receiptNumber, startsWith('RCPT-2026-'));
      expect(crcpt.contributionReceiptNumber, startsWith('CRCPT-2026-'));
      expect(
          rcpt.receiptNumber, isNot(equals(crcpt.contributionReceiptNumber)));
    });
  });
}
