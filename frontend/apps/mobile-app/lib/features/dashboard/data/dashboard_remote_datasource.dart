...
class DashboardRemoteDataSource {
  ...
  Future<MandalDashboardData> getMandalDashboard() async {
    return guardApiCall(() async {
      final response =
          await _dio.get<Map<String, dynamic>>('/dashboards/executive');
      final data = response.data!['data'] as Map<String, dynamic>;

      final activeVolunteers =
          (data['activeVolunteersCount'] as num?)?.toInt() ?? 0;
      final activeEvents = (data['activeEventsCount'] as num?)?.toInt() ?? 0;

      final totalCollectionsPaise =
          int.tryParse(data['totalCollectionsPaise']?.toString() ?? '0') ?? 0;
      final paidExpensesPaise =
          int.tryParse(data['paidExpensesPaise']?.toString() ?? '0') ?? 0;
      final netBalancePaise =
          int.tryParse(data['netLiquidityBalancePaise']?.toString() ?? '0') ??
              0;
      final todayTotalCollectedPaise =
          int.tryParse(data['todayTotalCollectedPaise']?.toString() ?? '0') ??
              0;
      ...
      try {
        final txResponse = await _dio.get<Map<String, dynamic>>(
            '/contributions',
          ...
          recentTransactions = list.map((item) {
            ...
            return TransactionItem(
              ...
              receiptNumber: m['receiptNumber'] as String? ??
                  'RCT-${m['id']?.toString().substring(0, 6)}',
              donorName: m['contributor']?['fullName'] as String? ??
                  m['donorName'] as String? ??
                  'Anonymous Donor',
              amountPaise: int.tryParse(amountStr) ?? 0,
              paymentMethod:
                  m['paymentMode'] as String? ?? m['mode'] as String? ?? 'UPI',
